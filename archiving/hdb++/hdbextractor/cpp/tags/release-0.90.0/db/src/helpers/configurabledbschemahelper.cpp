 ...
 *         examining the QueryConfiguration and help determine what to do with the settings
 *         stored into it.
 *
 */
...
/** \brief Returns the "data from the past" integration mode according to the QueryConfiguration
 *         options and the provided start_date.
 *
 *  @param queryConfiguration a pointer to the QueryConfiguration object
                                                     ...
                                                     const char *first_value_date) const
{
    ...
    if(!queryConfiguration->hasKey("FillFromThePastMode"))
        ...
        mode = KeepWindow;
    else if(queryConfiguration->get("FillFromThePastMode").compare("WidenWindow") == 0)
        mode = WidenWindow;
    else
        perr("ConfigurableDbSchemaHelper.fillFromThePastMode: unrecognized FillFromThePastMode \%s\"",
             queryConfiguration->get("FillFromThePastMode").c_str());

    if(mode != None)
    {
        double windowPercent = 5;
        bool ok = true;
        if(queryConfiguration->hasKey("FillFromThePastThresholdPercent"))
            windowPercent = queryConfiguration->getDouble("FillFromThePastThresholdPercent", &ok);
        ...
        if(!ok)
        {
            ...
            perr(" the \"FillFromThePastThresholdPercent\" option: \"%s\": must be a floating point number.",
             queryConfiguration->get("FillFromThePastThresholdPercent").c_str());
